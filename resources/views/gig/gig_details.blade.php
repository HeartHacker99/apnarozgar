            ...
            <div class="panel panel-default">
                ...
                @if(Auth::check())
                    @if(Auth::user()->role == 0)
                        <!-- Modal -->
                            ...
                            <button type="button" class="btn btn-info btn-lg" data-toggle="modal" data-target="#myModal">
                                Order Now (PKR {{ $gig->price }})
                            </button>
                            <a href="{{url('chatify')}}/{{$gig->user->id}}" class="btn btn-success btn-lg" >
                                Contact <i class="fas fa-comments"></i>
                            </a>
                    @elseif(Auth::user()->role == 1)

                        You need to <strong>login as Buyer</strong> to order this gig!
                    ...
                    <div style="margin-top: 30px">
                        {{--<div style="margin-top: 30px">
                            @if(Auth::user()->id)
                                <img src="{{Auth::user()->profile_photo_url}}" class="img-circle center-block" height="100" width="100">
                            @else
                                <img src="{{asset('/admin_assets/images/icon/avatar-02.jpg')}}" class="img-circle center-block" height="100" width="100">
                            @endif
                        </div>
                        <a href="{{ route('profile.show', $gig->user->name) }}">
                            <h4 class="text-center">{{ $gig->user->name }}</h4>
                        </a>--}}
                        @if(isset($gig->user->profile['avatar']) == null)
                            <img src="{{asset('/admin_assets/images/icon/avatar-06.jpg')}}" class="img-circle center-block" height="100" width="100">

                        @else
                            <img src="{{asset('/img/gigs/' . isset($gig->user->profile['avatar']))}}"
